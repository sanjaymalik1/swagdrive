...
import {
  PERSONALIZED_GIFTING_SOLUTION_TABS,
  type PersonalizedGiftingSolutionTab,
} from "./personalized-gifting-solutions-data";

function SolutionPanel({ tab }: { tab: PersonalizedGiftingSolutionTab }) {
  return (
...
export default function PersonalizedGiftingSolutionsSection() {
  const [activeId, setActiveId] = useState(
    PERSONALIZED_GIFTING_SOLUTION_TABS[0].id
  );
  const activeTab =
    PERSONALIZED_GIFTING_SOLUTION_TABS.find((tab) => tab.id === activeId) ??
    PERSONALIZED_GIFTING_SOLUTION_TABS[0];
        ...
        <div className="mx-auto w-full max-w-7xl">
          ...
          <div className="overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <div
              ...
              role="tablist"
              aria-label="Personalized gifting solutions"
            >
              {PERSONALIZED_GIFTING_SOLUTION_TABS.map((tab) => {
                ...
                return (
                  <button
                    ...
                    aria-controls={`pg-solution-panel-${tab.id}`}
                    id={`pg-solution-tab-${tab.id}`}
          ...
          <div
            role="tabpanel"
            id={`pg-solution-panel-${activeTab.id}`}
            aria-labelledby={`pg-solution-tab-${activeTab.id}`}
          >
